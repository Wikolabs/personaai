...
export default function DemoPage() {
  ...
  const t = lang === "fr" ? {
    back: "Retour", title: "Démo", sub: PRODUCT + ", buyer persona génère en temps réel",
    desc: "Renseignez les signaux observes sur un visiteur de votre site. Le moteur IA construit un persona détaillé avec scoring de propension et recommandations d'engagement. Aucune connexion réelle aux trackers, c'est un POC qui montre la logique de production.",
    ...
    behaviorLabel: "Comportement observe", behaviorPlaceholder: "ex: 3 visites en 5 jours, pricing page revue, whitepaper SOC2 lu jusqu'au bout, demande de démo abandonnée à l'etape 2",
    generate: "Générer le persona", generating: "Calcul en cours...",
    briefTitle: "Persona généré", emptyHint: "Le persona, scoring et recommandations apparaîtront ici.",
    sendSegment: "Pousser dans Segment", syncHubspot: "Sync vers HubSpot", retargetLinkedin: "Retargeter sur LinkedIn",
    sentMock: "Event \`persona_detected\` envoyé dans Segment (mode démo, pas de connexion réelle)",
    syncedMock: "Lead pousse dans HubSpot lifecycle MQL (mode démo, pas de connexion réelle)",
    retargetedMock: "Audience LinkedIn Ads créée : COO scale-up SaaS B2B (mode démo)",
    fallback: "Mode statique : la clé LLM sera ajoutée au prochain déploiement.",
    poweredBy: "Modèle :",
    note: "DEMO POC, aucune connexion réelle a Segment, HubSpot, LinkedIn Ads. L'IA imagine le persona pour la démonstration.",
  } : {
  ...
  return (
    <div style={{ minHeight: "100vh", background: PAL.bg, color: PAL.txt1, display: "flex", flexDirection: "column" }}>
      ...
      <main style={{ flex: 1, padding: "32px", maxWidth: 1200, margin: "0 auto", width: "100%" }}>
        ...
        <div className="demo-grid" style={{ display: "grid", gridTemplateColumns: "360px 1fr", gap: 24 }}>
          <section style={{ background: PAL.surface, border: `1px solid ${PAL.border}`, borderRadius: 6, padding: 22 }}>
            <h2 style={{ fontSize: "0.92rem", color: PAL.txt2, fontWeight: 700, margin: "0 0 14px" }}>{t.indLabel}</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 18 }}>
              <input className="wk-input" value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder={t.indPlaceholder} />
              <label style={{ fontSize: 11, color: PAL.txt3, fontWeight: 700, marginTop: 6 }}>{t.sizeLabel}</label>
              <input className="wk-input" value={companySize} onChange={(e) => setCompanySize(e.target.value)} placeholder={t.sizePlaceholder} />
              <label style={{ fontSize: 11, color: PAL.txt3, fontWeight: 700, marginTop: 6 }}>{t.behaviorLabel}</label>
      ...
      {toast && (
        <div style={{ position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)", background: "#FFFFFF", border: `1px solid ${PAL.accentBorder}`, borderRadius: 8, padding: "12px 20px", color: PAL.txt1, fontSize: 13, fontWeight: 600, zIndex: 50, backdropFilter: "blur(20px)", boxShadow: "0 8px 28px rgba(0,0,0,0.15)" }}>
          ✓ {toast}
        </div>
      )}
    </div>
